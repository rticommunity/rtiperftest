...
set "cStringifFile_script=%script_location%resource\perl_script\cStringifyFile.pl"
...
if !BUILD_CPP! == 1 (
	call !PERL! -version > nul 2>nul
	if not !ERRORLEVEL! == 0 (
		echo [WARNING]: !PERL! not found, !classic_cpp_folder!\qos_string.h will not be updated.
	) else (
		perl !cStringifFile_script! !qos_file! PERFTEST_QOS_STRING > !classic_cpp_folder!\qos_string.h
...
if !BUILD_CPP03! == 1 (
	...
	if not !ERRORLEVEL! == 0 (
		...
		perl !cStringifFile_script! !qos_file! PERFTEST_QOS_STRING > !modern_cpp_folder!\qos_string.h
