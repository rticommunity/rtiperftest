    ...
    const struct NDDS_WriterHistory_WriteParams * write_params)
{
    ...
    sample->parent.sample.coherentSetInfo = wh->_coherentSetInfo;
        ...
        const struct RTINtpTime * now_in)
{
    ...
    struct WriterHistoryFastMemory * wh = (struct WriterHistoryFastMemory *)history_in;
    struct WriterHistoryFastMemorySample *sample;
    struct NDDS_WriterHistory_WriteParams writeParams =
            NDDS_WRITER_HISTORY_WRITE_PARAMS_DEFAULT;
    ...
    sample->parent.sample.sn = wh->_nextSn;
    REDASequenceNumber_plusplus(&wh->_nextSn);
    sample->parent.kind = NDDS_WRITERHISTORY_BATCH_SAMPLE;
    sample->parent.sample.protocolParameters.pointer = NULL;
    sample->parent.sample.protocolParameters.length = 0;
    ...
    sample->parent.sample.timestamp = *timestamp_in;
    sample->parent.sample.coherentSetInfo = wh->_coherentSetInfo;
    sample->parent.sample.batchSampleInfoData =
            &sample->parent.sample.singleBatchSampleInfoData;
    sample->parent.sample.batchSampleInfoData->pointer = wh->_batchInfoBuffer;
    sample->parent.sample.batchSampleInfoData->length =
            wh->_batchInfoBufferLength;
    sample->parent.batchStatus = &wh->_batchStatus;
    sample->parent.batchStatus->batchEncapsulationKind = &wh->_batchEncapsulationKind;
    ...
    batch_out[0] = (struct NDDS_WriterHistory_Sample *)sample;
...
{
    RTI_INT32 failReason = NDDS_WRITERHISTORY_RETCODE_FAILURE_INTERNAL;
    struct WriterHistoryFastMemory *wh =
            (struct WriterHistoryFastMemory *) history_in;

    failReason = wh->_listener.close_batch(
            &wh->_listener,
            (struct NDDS_WriterHistory_Sample *) &wh->_sample);

    if (failReason != NDDS_WRITERHISTORY_RETCODE_OK) {
        return failReason;
    }

    sample_out[0] = (struct NDDS_WriterHistory_Sample *)&wh->_sample;
    *sampleCount_out = 1;
    *sessionId_out = 0;
    firstAvailableSn_out[0] = wh->_sample.parent.sample.sn;
    firstAvailableVirtualSn_out[0] = wh->_sample.parent.sample.sn;
        ...
        void * reserved)
{
    ...
    *history_out = NULL;
    ...
    if (wh->_property.batchEnabled) {
        RTIOsapiHeap_allocateBufferAligned(
                &wh->_serBuffer,
                wh->_property.batchDataSize.size,
                wh->_property.batchDataSize.alignment);
        ...
        RTIOsapiHeap_allocateBufferAligned(
                &wh->_batchInfoBuffer,
                wh->_property.batchMetaDataSize.size,
                wh->_property.batchMetaDataSize.alignment);
        
        if (wh->_batchInfoBuffer == NULL) {
            goto done;
        }

        wh->_serBufferLength = wh->_property.batchDataSize.size;
        wh->_serData.serializedData.pointer = wh->_serBuffer;
        wh->_batchInfoBufferLength = wh->_property.batchMetaDataSize.size;
    } else {
        RTIOsapiHeap_allocateBufferAligned(
                &wh->_serBuffer,
    ...
    wh->_coherentSetInfo = MIG_COHERENT_SET_INFO_UNKNOWN;
    ...
    if (property_in->storeFilterStatus ||
            property_in->maxRemoteReaderFilters ==
                    REDA_FAST_BUFFER_POOL_UNLIMITED) {
        ...
        struct NDDS_WriterHistory_Plugin ** plugin)
{
    ...
    *plugin = NULL;
    ...
    me->create_history = (NDDS_WriterHistory_Plugin_CreateHistoryFcn)WriterHistoryFastMemoryPlugin_createHistory;
    me->destroy_history = WriterHistoryFastMemoryPlugin_destroyHistory;
    me->add_sample = WriterHistoryFastMemoryPlugin_addSample;
    me->add_batch_sample_group = WriterHistoryFastMemoryPlugin_addBatchSampleGroup;
    me->add_historical_sample = NULL;
    me->find_sample = NULL;
    me->get_first_available_sn = (NDDS_WriterHistory_Plugin_GetFirstAvailableSnFcn)WriterHistoryFastMemoryPlugin_getFirstAvailableSn;
    me->get_last_available_sn = (NDDS_WriterHistory_Plugin_GetLastAvailableSnFcn)WriterHistoryFastMemoryPlugin_getLastAvailableSn;
    me->get_first_non_reclaimable_sn = (NDDS_WriterHistory_Plugin_GetFirstNonReclaimableSnFcn)WriterHistoryFastMemoryPlugin_getFirstNonReclaimableSn;
    ...
    me->prune_expired_samples = WriterHistoryMemoryPlugin_pruneExpiredSamples;
    ...
    me->get_app_ack_non_reclaimable_samples_count =
        NULL;
    me->flush_batch = WriterHistoryFastMemoryPlugin_flushBatch;
    ...
    me->get_batch_in_progress = WriterHistoryFastMemoryPlugin_getBatchInProgress;
    me->get_next_sn = (NDDS_WriterHistory_Plugin_GetNextSnFcn)WriterHistoryFastMemoryPlugin_getNextSn;
