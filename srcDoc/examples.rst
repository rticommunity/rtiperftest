...
Note how no ``-dataLen`` argument is specified. That's because, in the case of
Zero Copy v2, the length of the array has to be set at build time through the 
build script. That size is then appended to the name of the executable so that 
executables with different ZC data lengths can coexist. Set ``-executionTime 
<seconds>`` to >=100 for statistically better results.
...
uses unbounded sequences, *RTI Connext* will not pre-allocate the
...
might be a very high value, and the *RTI Connext* middleware might
...
*CFT* support is implemented only for *RTI Connext Professional*.
...
-  ``-nddshome``
   ...
   Path to the *RTI Connext* installation. If this parameter is not
