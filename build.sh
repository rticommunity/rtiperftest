...
function executable_checking()
{
    ...
    if [ "${BUILD_FASTDDS}" -eq "1" ]; then
        ...
        # For the time being, the RTIMEHOME variable is required.
        if [ -z "${RTIMEHOME}" ]; then
            echo -e "${ERROR_TAG} RTIMEHOME not set or path does not exist"
            exit -1
        else
...
function additional_defines_calculation_micro()
{
    ...
    additional_defines="RTI_LANGUAGE_CPP_TRADITIONAL PERTEST_RTI_MICRO O3"${additional_defines}
...
function additional_defines_calculation_micro_other_dds_impl()
{
    if [[ $platform == *"Linux"* ]]; then
        additional_included_libraries="nsl;rt;"
    fi
...
function build_micro_cpp()
{
    ...
    if [ "${BUILD_MICRO_24x_COMPATIBILITY}" -eq "1" ]; then
        additional_defines=${additional_defines}" PERTEST_RTI_MICRO_24x_COMPATIBILITY"
    else
...
{
    # This should be temporal until we get rid of micro for other middlewares.
    additional_defines_calculation_micro_other_dds_impl
...
function build_cycloneDDS_cpp()
{
    ...
    cmake_generate_command="${CMAKE_EXE} -DCMAKE_BUILD_TYPE=${RELEASE_DEBUG} -G \"Unix Makefiles\" -B./cycloneDDS_build -H. -DPLATFORM_LIBS=\"dl;m;pthread;${additional_included_libraries}\""
